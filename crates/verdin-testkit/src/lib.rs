//! Test helpers shared by integration tests. Not published.
...
impl TestDb {
    ...
    pub async fn exec(&self, sql: &str, params: &[SqlValue]) {
    ...
    pub async fn texts(&self, sql: &str) -> Vec<Option<String>> {
        let mut conn = self.db.acquire().await.unwrap();
        conn.fetch_all(sql, &[], &[ColumnKind::Text])
            .await
    ...
    pub async fn count(&self, table: &str) -> i64 {
        let mut conn = self.db.acquire().await.unwrap();
        let rows = conn
            .fetch_all(&format!("SELECT COUNT(*) FROM {table}"), &[], &[ColumnKind::BigInt])
            .await
            .unwrap();
    ...
    pub async fn insert(&self, table: &str, document_id: &str, columns: &[(&str, &str)]) {
        ...
        let mut params = vec![SqlValue::from(document_id)];
        params.extend(columns.iter().map(|(_, value)| SqlValue::from(*value)));
        self.exec(
            &format!(
